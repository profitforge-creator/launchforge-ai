...
import { MOCK_HISTORY, MOCK_USER } from "@/lib/mock-data";
import { formatRelativeDate } from "@/lib/utils";

function ScoreDot({ score }: { score: number }) {
  const color = score >= 80 ? "hsl(151 60% 48%)" : score >= 60 ? "hsl(38 90% 55%)" : "hsl(0 72% 58%)";
  return (
    <span className="font-semibold tabular-nums text-sm" style={{ color }}>
      {score}
    </span>
  );
}
  ...
  return (
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex items-center gap-2">
          <div className="text-right">
            ...
            <p className="text-sm font-semibold" style={{ color: "hsl(220 9% 88%)" }}>
              {MOCK_USER.generationsUsed} / {MOCK_USER.generationsLimit}
            </p>
          </div>
          <div
            className="w-10 h-10 rounded-lg flex items-center justify-center"
            style={{ backgroundColor: "hsl(220 13% 14%)", border: "1px solid hsl(220 13% 20%)" }}
          >
            <div className="relative w-6 h-6">
              <svg viewBox="0 0 36 36" className="w-6 h-6 -rotate-90">
                ...
                <circle
                  ...
                  stroke="hsl(213 94% 62%)" strokeWidth="3"
                  strokeDasharray={`${(MOCK_USER.generationsUsed / MOCK_USER.generationsLimit) * 94} 94`}
                  strokeLinecap="round"
                />
              </svg>
            </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Generations", value: MOCK_USER.generationsUsed.toString(), sub: "this month" },
          { label: "Best Score", value: "84", sub: "DevFlow · Jun 10" },
          { label: "Niches Explored", value: "5", sub: "across all time" },
        ].map((s) => (
          <div
            key={s.label}
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-sm font-semibold" style={{ color: "hsl(220 9% 88%)" }}>
              New Generation
            </h2>
      ...
      {recent.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold" style={{ color: "hsl(220 9% 75%)" }}>
              Recent generations
            </h2>
            <Link
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div
                    ...
                    style={{ backgroundColor: "hsl(220 13% 16%)", border: "1px solid hsl(220 13% 22%)" }}
                  >
                ...
                <div className="flex items-center gap-3 shrink-0 ml-4">
                  <Badge variant={r.status === "completed" ? "default" : "warning"}>
                    {r.status}
                  </Badge>
